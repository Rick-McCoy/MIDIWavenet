...
class Wavenet:
    ...
    def train(self, x, condition, target, step=1, train=True):
        ...
        if train:
            ...
            if step % 20 == 19:
                ...
                self.writer.add_image('Score/Generated', torch.nn.Softmax(output[0].unsqueeze(dim=0)), step)
        del loss
        return loss_item
    ...
    def generate(self, temperature=1., init=None, condition=None):
        ...
        inc = 0
        while True:
            cont = torch.nn.Softmax(self.net.module.sample_forward(output[:, -2:].unsqueeze(dim=0), condition))
